				...
				if [ -d "$dir" ]; then
				   ...
				   #Check for the priv directory
					if [ -d "$priv" ]; then
					    ...
					    echo
					    cp -Rv /mnt/qadmin/users/skel/.defaultbashrc /mnt/qadmin/users/$username/priv/.bash_profile
					    echo
				...
				if [ -d "$dir" ]; then
				   ...
				   #Check for the priv directory
					if [ -d "$priv" ]; then
					    echo The $priv folder exists.
					    echo
					    cp -Rv /mnt/qadmin/users/skel/.defaultbashrc_ddl /mnt/qadmin/users/$username/priv/.bash_profile
					    echo
...
# if the input username is DDL, then set up that person using ddl specific files, otherwise set them up as a standard user

	if [[ $username =~ $ddl_users_regex ]] ; then
