...
do
	MEM_CHECK=$(qhost \
		...
		| awk '$1~"c6100-"&&$5<94 {print $1 , "HAS" , $5 , "Gb RAM, WHICH IS LESS THAN IT SHOULD HAVE. PLEASE DRAIN, REBOOT AND UNDRAIN" ";"} \
			...
			$1~"c6320-"&&$5<252 {print $1 , "HAS" , $5 , "Gb RAM, WHICH IS LESS THAN IT SHOULD HAVE. PLEASE DRAIN, REBOOT AND UNDRAIN" ";"} \
	...
	ONLINE_CHECK=$(qstat -f -s r \
		| egrep -v "^-|^[0-9]|^ " \
		| sed -r 's/[[:space:]]+/\t/g' \
		| awk 'BEGIN {FS="\t"};{OFS="\t"} $6=="au" {print $1, "IS IN AN ALERT/UNREACHABLE STATUS. CHECK, REBOOT IF NEEDED"}') ;
